...
let values:string|number="Hello TypeScript"; // can hold string [[[[[ or ]]]]] number
values=25;
console.log(values);
...
let Mix:(string|number)[]=["Hello",25,"World",30];

// tuple
let student:[string,number]=["Alice",25];

let anyType:any[]=["Hello",25,true,["Alice","Bob"],{name:"Alice",age:25}];


let user:string | number[]="Hello TypeScript";  // can hold string or number array [[ not ]] array of string or number
user=[5, 60, 70];
...
function getUserInfoWithInterface(){
    const user:{name:string,age:number}={name:"Alice",age:26};
...
function PrintGenericArray<T>(arr:T[]):void{
    arr.forEach(item=>{
        console.log(item);
    });
}

let stdNames:string[]= ["Alice","Bob","Charlie"];
let stdAges:number[] = [25,30,35];

PrintGenericArray<string>(stdNames);  // == > PrintGenericArray(stdNames);
PrintGenericArray<number>(stdAges);


function DeleteFromArray<T>(arr:T[]):T[]{
...
let namesToDelete:string[]=["Alice","Bob","Charlie"];
console.log(DeleteFromArray(namesToDelete));

let agesToDelete:number[]=[25,30,35];
...
// 2- Interfaces [Template With No Implementation -->> only declaration of properties and methods]
...
let productWithInterface:products={
...
let productWithInterface2:products[]=[
...
// shop has all properties of products interface and can have additional properties if needed

interface Shop extends products{
    description:string;
}
...
interface User{
    ...
    printInfo(len:number):string; //return type must be specified for methods in interfaces
}
...
// Interfaces do not appear in the generated JavaScript file.

// When creating a variable using an interface type,
// it becomes a normal JavaScript object at runtime
// because JavaScript does not support interfaces.
...
let x : string | number| boolean ="Ahmed"
let y : string | number| boolean =25
let z : string | number| boolean =true

// Duplicate code -->>  string | number| boolean 

// Solution -->> Type Aliases

type customType = string | number| boolean;

let x2 : customType ="Ahmed"
let y2 : customType =25
let z2 : customType =true
...
// Enums convert to JavaScript objects at runtime

// Can Use Type Aliases to define a set of named constants that can be either numeric or string values. 

type userRoleType = "Admin" | "User" | "Guest";

let role2:userRoleType = "Admin";
